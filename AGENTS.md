...
4. **先小后大**
   ...
   - 旧的 `v1 不做 GUI` 已被当前 Phase 30 显式覆盖为：
     - 允许做 local-first GUI / Web UI decision desk
     - 但仍然不做 hosted / browser-executor / cloud backend
...
- integration pack 默认按 repo-local MCP bridge 理解
  ...
  - 再尝试 `cargo run --manifest-path <repo>/Cargo.toml --bin AgentExport --`
  ...
  - `AgentExport evidence diff --left <report> --right <report>` 现在能解释 readiness、check、next-step 的变化
...
- hosted / browser-side Web UI 平台壳
