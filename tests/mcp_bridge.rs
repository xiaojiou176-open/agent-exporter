...
fn agent-export_bin() -> PathBuf {
    PathBuf::from(env!("CARGO_BIN_EXE_AgentExport"))
}
...
fn mcp_bridge_lists_tools_and_can_publish_archive_index() {
    ...
    let mut child = Command::new(python_command())
        .arg(mcp_script_path())
        .env("AGENT_EXPORT_BIN", agent-export_bin())
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
...
fn mcp_bridge_reads_integration_evidence_tools() {
    ...
    let onboard = Command::new(agent-export_bin())
    ...
    let doctor = Command::new(agent-export_bin())
        .current_dir(workspace.path())
        .args([
            "doctor",
            "integrations",
    ...
    let baseline_promote = Command::new(agent-export_bin())
        .current_dir(workspace.path())
        .args([
            "evidence",
            "baseline",
    ...
    let promote_attempt = Command::new(agent-export_bin())
        .current_dir(workspace.path())
        .args([
            "evidence",
            "promote",
...
fn mcp_bridge_reads_governance_tools() {
    ...
    let onboard_again = Command::new(agent-export_bin())
        .current_dir(workspace.path())
        .args([
            "onboard",
            "codex",
    ...
    let promote = Command::new(agent-export_bin())
        .current_dir(workspace.path())
        .args([
            "evidence",
            "promote",
