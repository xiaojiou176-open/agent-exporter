...
fn print_scaffold_status() {
    println!("AgentExport scaffold status");
    println!(
    ...
    println!("- Real Codex export path: `AgentExport export codex --thread-id <id>`.");
    println!(
        "- Real Claude export path: `AgentExport export claude-code --session-path <path>`."
    );
    ...
    println!(
        "- Real archive index path: `AgentExport publish archive-index --workspace-root <repo>`; this now also writes `index.json`, workspace timeline, family case views, official answer lifecycle state, and share-safe packet tiers (`share-safe-packet.md`, `.teammate`, `.reviewer`, `.public`)."
    );
    println!(
        "- Real export cockpit path: `AgentExport ui cockpit --workspace-root <repo>`; it now supports Codex threads plus workspace-local Claude session discovery."
    );
    println!(
        "- Real official answer workflow path: `AgentExport publish pin-answer --workspace-root <repo> --artifact <summary-or-report.json> --label <name>` plus `resolve-answer` / `unpin-answer` for lifecycle closure."
    );
    println!(
        "- Real reports shell path: generated by `AgentExport publish archive-index --workspace-root <repo>` into `.agents/Search/Reports/index.html`."
    );
    println!(
        "- Real decision desk path: generated by `AgentExport publish archive-index --workspace-root <repo>` into `.agents/Conversations/index.html`, using saved integration evidence plus baseline/policy/history governance artifacts as a read-only decision panel."
    );
    println!(
        "- Real semantic retrieval path: `AgentExport search semantic --workspace-root <repo> --query <text>`."
    );
    println!(
        "- Real hybrid retrieval path: `AgentExport search hybrid --workspace-root <repo> --query <text>`."
    );
    println!(
        "- Real MCP bridge path: `python3 scripts/agent-export_mcp.py` with local stdio tool exposure for publish/search/evidence/governance read-only workflows."
    );
    println!(
        "- Real integration materializer path: `AgentExport integrate <platform> --target <dir>`."
    );
    println!(
        "- Real integration doctor path: `AgentExport doctor integrations --platform <platform> --target <dir>`."
    );
    println!("- Real onboarding path: `AgentExport onboard <platform> --target <dir>`.");
    println!(
        "- Real integration evidence path: `AgentExport doctor integrations --platform <platform> --target <dir> --save-report` or `AgentExport onboard <platform> --target <dir> --save-report`."
    );
    println!(
        "- Real evidence diff path: `AgentExport evidence diff --left <report.json|html> --right <report.json|html>`."
    );
    println!(
        "- Real evidence gate path: `AgentExport evidence gate --baseline <report.json|html> --candidate <report.json|html>`."
    );
    println!(
        "- Real evidence explain path: `AgentExport evidence explain --report <report.json|html>`."
    );
    println!(
        "- Real remediation bundle path: `AgentExport evidence remediation --report <report.json|html>`."
    );
    println!(
        "- Real baseline registry path: `AgentExport evidence baseline list|show|promote` backed by `.agents/Integration/Reports/baseline-registry.json`."
    );
    println!(
        "- Real policy pack path: `AgentExport evidence policy list|show` backed by `policies/integration-evidence/*.json`."
    );
    println!(
        "- Real decision promotion path: `AgentExport evidence promote --candidate <report> --baseline-name <name>`."
    );
    println!(
        "- Real decision history path: `AgentExport evidence history` backed by `.agents/Integration/Reports/decision-history.json`."
    );
    println!(
        "- Real current-decision path: `AgentExport evidence current --baseline-name <name>`."
    );
    println!(
        "- Next step: a new post-public-surface-prep product decision, while staying local-first and non-hosted."
    );
}
...
fn integration_reports_title(workspace_root: &std::path::Path) -> String {
    workspace_root
        ...
        .unwrap_or_else(|| "AgentExport integration reports".to_string())
}
