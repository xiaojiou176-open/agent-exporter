...
fn print_scaffold_status() {
    println!("BeamMe scaffold status");
    println!(
    ...
    println!("- Real Codex export path: `BeamMe export codex --thread-id <id>`.");
    println!(
        "- Real Claude export path: `BeamMe export claude-code --session-path <path>`."
    );
    ...
    println!(
        "- Real archive index path: `BeamMe publish archive-index --workspace-root <repo>`; this now also writes `index.json`, workspace timeline, family case views, official answer lifecycle state, and share-safe packet tiers (`share-safe-packet.md`, `.teammate`, `.reviewer`, `.public`)."
    );
    println!(
        "- Real export cockpit path: `BeamMe ui cockpit --workspace-root <repo>`; it now supports Codex threads plus workspace-local Claude session discovery."
    );
    println!(
        "- Real official answer workflow path: `BeamMe publish pin-answer --workspace-root <repo> --artifact <summary-or-report.json> --label <name>` plus `resolve-answer` / `unpin-answer` for lifecycle closure."
    );
    println!(
        "- Real reports shell path: generated by `BeamMe publish archive-index --workspace-root <repo>` into `.agents/Search/Reports/index.html`."
    );
    println!(
        "- Real decision desk path: generated by `BeamMe publish archive-index --workspace-root <repo>` into `.agents/Conversations/index.html`, using saved integration evidence plus baseline/policy/history governance artifacts as a read-only decision panel."
    );
    println!(
        "- Real semantic retrieval path: `BeamMe search semantic --workspace-root <repo> --query <text>`."
    );
    println!(
    ...
    println!(
        "- Real integration doctor path: `BeamMe doctor integrations --platform <platform> --target <dir>`."
    );
    println!("- Real onboarding path: `BeamMe onboard <platform> --target <dir>`.");
    println!(
        "- Real integration evidence path: `BeamMe doctor integrations --platform <platform> --target <dir> --save-report` or `BeamMe onboard <platform> --target <dir> --save-report`."
    );
    println!(
        "- Real evidence diff path: `BeamMe evidence diff --left <report.json|html> --right <report.json|html>`."
    );
    println!(
        "- Real evidence gate path: `BeamMe evidence gate --baseline <report.json|html> --candidate <report.json|html>`."
    );
    println!(
        "- Real evidence explain path: `BeamMe evidence explain --report <report.json|html>`."
    );
    println!(
        "- Real remediation bundle path: `BeamMe evidence remediation --report <report.json|html>`."
    );
    println!(
        "- Real baseline registry path: `BeamMe evidence baseline list|show|promote` backed by `.agents/Integration/Reports/baseline-registry.json`."
    );
    println!(
        "- Real policy pack path: `BeamMe evidence policy list|show` backed by `policies/integration-evidence/*.json`."
    );
    println!(
        "- Real decision promotion path: `BeamMe evidence promote --candidate <report> --baseline-name <name>`."
    );
    println!(
        "- Real decision history path: `BeamMe evidence history` backed by `.agents/Integration/Reports/decision-history.json`."
    );
    println!(
        "- Real current-decision path: `BeamMe evidence current --baseline-name <name>`."
    );
    println!(
...
fn integration_reports_title(workspace_root: &std::path::Path) -> String {
    workspace_root
        ...
        .unwrap_or_else(|| "BeamMe integration reports".to_string())
}
