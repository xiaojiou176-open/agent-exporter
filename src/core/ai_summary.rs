...
fn ai_summary_capture_paths(thread_id: &str) -> (PathBuf, PathBuf) {
    ...
    (
        root.join(format!(
            "AgentExport-ai-summary-{slug}-{stamp}.stdout.log"
        )),
        root.join(format!(
            "AgentExport-ai-summary-{slug}-{stamp}.stderr.log"
        )),
    )
}
...
fn render_summary_html_document(
    ...
    format!(
        concat!(
            ...
            "  <meta name=\"AgentExport:summary-kind\" content=\"ai-summary\">\n",
            "  <meta name=\"AgentExport:thread-id\" content=\"{thread_id}\">\n",
            "  <meta name=\"AgentExport:thread-display-name\" content=\"{title}\">\n",
            "  <meta name=\"AgentExport:exported-at\" content=\"{exported_at}\">\n",
            "  <style>\n",
