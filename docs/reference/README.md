...
This folder is the maintainer-only reference layer for `AgentExport`.
...
- integration evidence timeline/diff via `AgentExport evidence diff` 已经 landed，不再是 future plan
- evidence gate / explain 已经 landed，不再是 future plan
- baseline registry via `AgentExport evidence baseline ...` 已经 landed，不再是 future plan
- policy packs via `AgentExport evidence policy ...` 已经 landed，不再是 future plan
- decision promotion/history via `AgentExport evidence promote` / `evidence history` 已经 landed，不再是 future plan
- remediation bundle studio via `AgentExport evidence remediation` 已经 landed，不再是 future plan
...
4. `codex-thread-archive-blueprint.md`
  - 当前 `AgentExport` 的产品/架构蓝图，明确 multi-connector 命令面、默认 source、输出格式顺序、以及 `degraded` 状态语义
